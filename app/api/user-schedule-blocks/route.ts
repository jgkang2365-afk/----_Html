...
import { parseDateOnly } from "@/lib/preliminary-survey/calendar";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const manageable = await Promise.all(
      userIds.map((userId) => canManageUser(supabase, session, userId)),
    );
    ...
    const message =
      error instanceof Error ? error.message : "SCHEDULE_BLOCK_CREATE_FAILED";
    return NextResponse.json(
      { error: message },
      { status: errorStatus(message) },
    );
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!Number.isInteger(id) || validationError) {
      return NextResponse.json(
        { error: validationError || "INVALID_BLOCK_ID" },
        { status: 400 },
      );
    }
    const supabase = await createClient();
    ...
    const message =
      error instanceof Error ? error.message : "SCHEDULE_BLOCK_UPDATE_FAILED";
    return NextResponse.json(
      { error: message },
      { status: errorStatus(message) },
    );
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "SCHEDULE_BLOCK_DELETE_FAILED";
    return NextResponse.json(
      { error: message },
      { status: errorStatus(message) },
    );
