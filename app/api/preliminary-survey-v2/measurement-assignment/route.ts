...
function assignmentError(message: string, approvalReview?: ThirdAssignmentReview | null) {
  ...
  if (message.includes("MEASUREMENT_ASSIGNMENT_HARD_MAX_EXCEEDED")) {
    return NextResponse.json({ error: "같은 측정일 동일 담당자 4건 이상은 배정할 수 없습니다.", code: "MEASUREMENT_ASSIGNMENT_HARD_MAX_EXCEEDED" }, { status: 409 });
  }
...
/** 비찐확정 사업장의 저장된 날짜별 공시료 담당자만 최소 수정한다. */
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (approveThirdAssignment && session.role !== "관리자") {
