...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: targets, error: targetError } = await supabase
      ...
      .select("journal_id, code, business_name, business_number, address, office_jurisdiction, latitude, longitude, geocoding_status, geocoding_error, measurement_date, measurement_end_date, year, period, sync_status")
      .eq("year", year)
      .eq("period", period)
    ...
    const journalIds = (targets || []).map((row: any) => row.journal_id).filter((id: unknown) => id != null);
    const [infoResult, journalResult, laborOfficeDirectory] = await Promise.all([
      ...
      journalIds.length
        ? supabase.from("measurement_journal").select("id, code, measurement_year, measurement_period").in("id", journalIds)
        : Promise.resolve({ data: [], error: null }),
