...
export default function ReportProcessingPage() {
    ...
    const handleExplorerSearch = async () => {
        ...
        try {
            const results = await searchReportExplorer({
                year: Number(explorerYear),
            ...
            <Card className="p-4 space-y-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">
                            {selectedRecords.length > 0
                                ? `선택된 보고서 처리 결과 ${selectedRecords.length}건의 사업장명을 사용합니다.`
                                : `현재 보고서 처리 결과 ${records.length}건의 사업장명을 사용합니다.`}
                        </p>
                    </div>
                ...
                <div className="grid gap-3 lg:grid-cols-[12rem_12rem_minmax(0,1fr)_auto] lg:items-end">
                    <Select
                        label="탐색 연도"
                        value={explorerYear}
                        onChange={(event: React.ChangeEvent<HTMLSelectElement>) => setExplorerYear(event.target.value)}
                        disabled={filters.year !== 'all'}
                        options={[
                            { value: '', label: filters.year === 'all' ? '연도 선택' : '연도 동기화 중' },
                            { value: '2024', label: '2024년' },
                            { value: '2025', label: '2025년' },
                            { value: '2026', label: '2026년' }
                        ]}
                    ...
                    <Select
                        ...
                        onChange={(event: React.ChangeEvent<HTMLSelectElement>) => {
                            ...
                            setExplorerPeriod(period === '상반기' || period === '하반기' ? period : '');
                        }}
                        disabled={filters.period !== 'all'}
                        options={[
                            { value: '', label: filters.period === 'all' ? '주기 선택' : '주기 동기화 중' },
                            { value: '상반기', label: '상반기' },
                            { value: '하반기', label: '하반기' }
                        ]}
                    />
                    <div className="space-y-1">
                        <label htmlFor="report-explorer-manual-names" className="block text-sm font-medium text-text-700">
                            사업장명 직접 추가
                        </label>
                        <textarea
                            ...
                            rows={2}
                            className="w-full resize-y rounded-lg border border-slate-200 px-3 py-2 text-sm leading-5 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-primary-500"
                        />
                    </div>
                    <Button
                        type="button"
                        variant="primary"
                        onClick={() => void handleExplorerSearch()}
                        disabled={explorerSearching || explorerOpeningResultId !== null}
                    >
                ...
                <Checkbox
                    ...
                    onChange={(event: React.ChangeEvent<HTMLInputElement>) => setUseReportProcessingResults(event.target.checked)}
                    label="현재 보고서 처리 결과 사용"
                />
                ...
                {explorerResults.length > 0 && (
                    <Table className="text-sm">
                        <TableHeader>
                            <TableRow>
                                <TableHead>검색 사업장</TableHead>
                                <TableHead>일치 사업장 폴더</TableHead>
                                <TableHead>경로</TableHead>
                                <TableHead className="text-center">상태</TableHead>
                                <TableHead className="text-center">동작</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {explorerResults.flatMap((result) => {
                                if (result.matches.length === 0) {
                                    return [
                                        <TableRow key={`${result.query}-${result.status}`}>
                                            ...
                                            <TableCell className="text-center">
                                                <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-1 text-xs font-medium text-slate-600">
                                                    {reportExplorerStatusLabel(result.status)}
                                                </span>
                                            </TableCell>
                                ...
                                return result.matches.map((match) => (
                                    <TableRow key={match.resultId}>
                                        <TableCell className="font-medium">{result.query}</TableCell>
                                        <TableCell title={match.folderName}>{match.folderName}</TableCell>
                                        <TableCell className="max-w-[32rem] truncate" title={match.path}>{match.path}</TableCell>
                                        <TableCell className="text-center">
                                            <span className={`rounded-full border px-2 py-1 text-xs font-medium ${result.status === 'FOUND'
                                        ...
                                        <TableCell className="text-center">
                                            <Button
                                                ...
                                                variant="secondary"
                                                size="sm"
                                                onClick={() => void handleExplorerOpen(match.resultId)}
                                                disabled={explorerOpeningResultId !== null || explorerSearching}
                                            >
                                                {explorerOpeningResultId === match.resultId
                                                    ? <Loader2 className="h-4 w-4 animate-spin" />
                                                    : <><ExternalLink className="mr-1 h-4 w-4" />열기</>}
                                            </Button>
                                        </TableCell>
                                    </TableRow>
                                ));
