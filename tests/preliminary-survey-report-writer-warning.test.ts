...
describe("보고서 담당자 측정 참여 경고", () => {
  ...
  it("다일 중 하루라도 해당 일자의 보고서 담당자가 참여하면 경고하지 않는다", () => {
    assert.equal(reportWriterParticipationWarning({ source: { dailyStaff: [{ date: "2026-08-27", measurer_id: 2, collaborators: ["참여자"] }, { date: "2026-08-28", measurer_id: 1, collaborators: ["보고서"] }] }, userIdByName: users }), null);
  });
  it("다일 전체에서 보고서 담당자가 한 번도 참여하지 않으면 경고한다", () => {
    assert.equal(reportWriterParticipationWarning({ source: { dailyStaff: [{ date: "2026-08-27", measurer_id: 1, collaborators: ["참여자"] }, { date: "2026-08-28", measurer_id: 1, collaborators: ["참여자"] }] }, userIdByName: users }), REPORT_WRITER_PARTICIPATION_WARNING);
  });
  it("다일 writer/participant가 서로 교차해도 같은 날 일치가 없으면 경고한다", () => {
    assert.equal(reportWriterParticipationWarning({ source: { dailyStaff: [{ date: "2026-08-27", measurer_id: 1, collaborators: ["참여자"] }, { date: "2026-08-28", measurer_id: 2, collaborators: ["보고서"] }] }, userIdByName: users }), REPORT_WRITER_PARTICIPATION_WARNING);
  });
  ...
  it("보고서 담당자 미지정은 참여 누락으로 오판하지 않는다", () => {
