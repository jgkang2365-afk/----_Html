...
test("Node WorkerDaemon은 background_jobs를 저빈도로 확인하고 오래된 작업 복구를 제한한다", () => {
  ...
  assert.match(source, /JOB_POLL_INTERVAL_MS = 15_000/);
  assert.match(source, /STALE_JOB_RECOVERY_INTERVAL_MS = 5 \* 60 \* 1000/);
  assert.match(source, /setInterval\(\(\) => void this\.poll\(\), JOB_POLL_INTERVAL_MS\)/);
...
test("Worker 로그와 Realtime 설정은 비밀값을 출력하지 않는다", () => {
  const worker = readFileSync("document_worker.py", "utf8");
  const runtime = readFileSync("document_worker_realtime.py", "utf8");
  assert.match(worker, /masked_supabase_url\(supabase_url\)/);
  assert.doesNotMatch(worker, /LOGGER\.[a-z]+\([^\n]*realtime_key/);
  assert.doesNotMatch(runtime, /LOGGER\.[a-z]+\([^\n]*apikey/);
});
test("트레이는 문서 Worker 전용 가상환경을 우선 사용한다", () => {
...
test("Python 3.14 Worker는 충돌을 피한 전용 websockets 의존성을 사용한다", () => {
  ...
  assert.match(requirements, /^websockets==16\.1\.1$/m);
  assert.doesNotMatch(requirements, /^realtime[=<>]/m);
});
