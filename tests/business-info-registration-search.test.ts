...
test("건강디딤돌 신청용 담당자명에서 직책만 제거한다", () => {
  assert.equal(normalizeContactName("남주원 이사"), "남주원");
  assert.equal(normalizeContactName("과장 홍길동"), "홍길동");
  assert.equal(normalizeContactName("김철수(부장)"), "김철수");
