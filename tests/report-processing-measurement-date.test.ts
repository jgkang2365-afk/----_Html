...
test('보고서 처리 측정일은 실시 target의 단일 실제 일자를 사용한다', () => {
  ...
  assert.equal(matchesReportProcessingMeasurementDateRange(dates, '2026-03-25', '2026-03-25'), true);
...
test('다일 측정은 daily_staff에 명시된 비연속 실제 일자만 필터·표시한다', () => {
  ...
  assert.equal(matchesReportProcessingMeasurementDateRange(dates, '2026-03-26', '2026-03-28'), true);
  assert.equal(matchesReportProcessingMeasurementDateRange(dates, '2026-03-26', '2026-03-26'), false);
...
test('보고서 처리 API는 target 일정을 batch 조회하고 날짜 범위를 정확히 필터한다', () => {
  const route = readFileSync('app/api/report-processing/route.ts', 'utf8');

  assert.match(route, /searchParams\.get\('measurementDateFrom'\)/);
  assert.match(route, /searchParams\.get\('measurementDateTo'\)/);
  assert.match(route, /searchParams\.get\('k2bReceiptDateFrom'\)/);
  assert.match(route, /searchParams\.get\('k2bReceiptDateTo'\)/);
  ...
  assert.match(route, /matchesReportProcessingMeasurementDateRange\(measurement_dates, measurementDateFrom, measurementDateTo\)/);
  assert.match(route, /\.gte\('k2b_verified_send_date', k2bReceiptDateFrom\)/);
  assert.match(route, /\.lte\('k2b_verified_send_date', k2bReceiptDateTo!\)/);
...
test('보고서 처리 화면은 날짜 범위를 조회 순서와 결과 표에 표시하고 현재 페이지만 선택한다', () => {
  ...
  assert.match(page, /날짜 불일치: 내부 \$\{record\.k2b_send_date\} \/ 실제 \$\{record\.k2b_verified_send_date\}/);
});
