...
test("기존 지도 모달에서도 출발지 기준 주변 업체를 조회하고 추가한다", () => {
  assert.match(legacyMapModal, /주변 업체 조회/);
  assert.match(legacyMapModal, /\/api\/businesses\/nearby/);
  assert.match(legacyMapModal, /distanceKm\.toFixed\(1\)/);
  assert.match(legacyMapModal, /handleAddBusiness\(business\)/);
});