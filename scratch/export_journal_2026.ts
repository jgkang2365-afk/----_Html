...
async function exportJournalExcel2026() {
  console.log('Fetching 2026 journals...');
  const { data: journals, error } = await supabase
    .from('measurement_journal')
    .select('*')
    .eq('measurement_year', 2026) // 2026년 데이터만 필터링
  ...
  const fileName = `측정일지_추출_2026년도_${new Date().toISOString().split('T')[0]}.xlsx`;
  const filePath = path.resolve(process.cwd(), fileName);
  XLSX.writeFile(wb, filePath);
  
  console.log(`Excel file created: ${fileName}`);
  console.log(`Total 2026 records: ${journals.length}`);
}

exportJournalExcel2026();
