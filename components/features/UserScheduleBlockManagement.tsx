...
export function UserScheduleBlockManagement() {
  ...
  useEffect(() => {
    void load().catch((caught) => setError(caught instanceof Error ? caught.message : "조회 실패"));
  }, [load]);
        ...
        <div className="grid gap-3 xl:grid-cols-[minmax(620px,2.8fr)_minmax(135px,0.8fr)_minmax(135px,0.8fr)_minmax(130px,0.7fr)_minmax(180px,1fr)_auto] xl:items-end">
          ...
          <div className="flex items-end gap-2">
            <Button disabled={saving} onClick={() => void save()}>
              {saving ? "저장 중..." : form.id ? "수정 저장" : `${form.userIds.length || ""}명 등록`}
            </Button>
      ...
      <Card className="p-5">
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="grid gap-2 sm:grid-cols-3">
            ...
            />
            <Input label="조회 시작일" type="date" value={filterStartDate} onChange={(event) => setFilterStartDate(event.target.value)} />
            <Input label="조회 종료일" type="date" value={filterEndDate} onChange={(event) => setFilterEndDate(event.target.value)} />
          </div>
        </div>
