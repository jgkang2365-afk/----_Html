...
export default function RemoteJobProgressDialog({ title, view, running, onClose, onCancel, cancelLabel = "작업 중단", cancelPending = false }: { title: string; view: RemoteJobProgressView; running: boolean; onClose: () => void; onCancel?: () => void | Promise<void>; cancelLabel?: string; cancelPending?: boolean }) {
  const canCancel = running && Boolean(onCancel) && !cancelPending;
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-[1px]" role="dialog" aria-modal="true" aria-label={title}>
    <div className="relative w-full max-w-[620px] rounded-2xl border border-slate-200 bg-white px-7 py-8 shadow-2xl sm:px-10 sm:py-12">
