...
set "MesDaemonBat=%~dp0run-mes-daemon.bat"
...
echo.
echo [안내] MES 동기화 데몬을 최고 권한 작업으로 등록하고 있습니다...
schtasks /delete /tn "%MesTaskName%" /f >nul 2>&1
schtasks /create /tn "%MesTaskName%" /sc onlogon /ru "%USERDOMAIN%\%USERNAME%" /rl highest /it /tr "%MesDaemonBat%" /f
...
echo.
echo MES 동기화 데몬도 최고 권한으로 자동 실행되어 요청을 대기합니다.
