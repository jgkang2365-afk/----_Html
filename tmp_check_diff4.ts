...
async function checkDiff() {
  ...
  for (const item of data) {
    ...
    if (shortName === "천안") {
      ...
      if (total !== bFee + nFee) {
        console.log("Difference found at ID", item.id, item.business_name, "Total:", total, "b+n:", bFee+nFee);
      }
    }
  }
  
  console.log(`2026년 천안 측정비 1번표 합계(totalFee): ${totalFee}`);
  console.log(`2026년 천안 측정비 2번표 합계(bFee+nFee): ${totalSub}`);
}
