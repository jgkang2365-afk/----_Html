...
### 3차 보완 — 목록 화면의 예비조사 추천/묶음 추천/V2 plan 표시 제거

`components/features/MeasurementTargetBusinessManagement.tsx`에서 **목록 화면의 예비조사 UI/추천 기능 전체 제거** (-339줄).

- 제거: `handlePreliminarySurveyRecommendation`(개별/선택 추천 호출), `recommendingTargetIds` state
- 제거: `openGroupRecommendation`/`toggleGroupTarget`/`handleGroupConfirm`(묶음 추천/확정) + GroupRec 타입/state
- 제거: 목록 상단 "선택 사업장 예비조사 추천"·"예비조사 일정 추천 (묶음)" 버튼
- 제거: 목록 헤더 "예비조사 추천" 컬럼 + 행별 V2 plan 표시(추천일/예비조사자)·추천 버튼
- 제거: 묶음 추천 모달(주소 기반 그룹 선택/확정 UI)
