...
예비조사 V2는 실시간 현장 일정 스케줄러가 아니라, 실제 수행되는 예비조사 업무를 전제로 측정 건의 예비조사 서류를 내부 기준에 맞게 정합성 있게 구성·보완하는 시스템이다.
...
5. 측정자(공시료) 균등 배정
6. 초과 업체의 측정자 추가 배정
7. document integrity 재검증과 operational feasibility 참고정보 계산
8. 짧은 추천 사유 생성
9. 사용자 검토
10. 적용
...
- 기존업체: working day `-20, -19 ... -3`을 먼저 탐색하고 불가능할 때 `-25, -24 ... -21`을 검토하며 유선으로 분류한다.
- 휴일·주말은 추천 후보에서 제외한다.
- 측정예정일보다 앞선 정책 범위의 영업일이면 오늘보다 과거이거나 측정예정일 자체가 오늘·과거여도 추천할 수 있다.
...
- 사업장 배열 index나 직원 배열 순번으로 예비조사자를 round-robin 배정하지 않는다. 업체별 후보 날짜마다 가능한 단독 조사자와 허용 조합을 만들고 기존 배정 균형·역할 연계·결정적 ID 순서를 preference로 적용한다.
- 신규 방문은 경력자 단독 또는 경력자+비경력자만 허용한다. 기존업체 유선은 1인 배정을 기본으로 하며 신규 방문의 경력 hard constraint를 그대로 적용하지 않는다.
- 업체별 재추천은 같은 예비조사일·조사자·주소와 다일을 포함한 같은 실제 측정일의 공시료 균형 관계를 참고할 수 있다. 찐확정은 관계 계산에는 포함하되 변경 대상에서는 제외한다.
...
- 기존 manual plan도 현재 document integrity hard rule을 다시 통과한 경우에만 minimum-change 대상으로 보존한다. 기존업체 유선은 경력 여부와 무관한 1인 배정을 허용한다.
- 같은 예비조사일의 방문·유선 건수나 참여자 겹침은 dependency 또는 저장 차단 조건으로 만들지 않는다. 같은 실제 측정일의 측정자·공시료 균형 관계는 별도 배정 transaction에서만 유지한다.
