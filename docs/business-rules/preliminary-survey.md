...
- 보고서 담당자가 지정된 경우 그 담당자가 실제 측정 참여자에 포함되는 것이 정상이다. 단일일 측정에서 포함되지 않으면 `보고서 담당자 측정 참여 누락` warning을 표시하되 추천·저장을 차단하지 않는다. 다일 측정은 authoritative `daily_staff` 전체 중 하루라도 해당 일자의 보고서 담당자가 실제 참여자로 포함되면 warning을 표시하지 않는다. 보고서 담당자 미지정은 참여 누락으로 보지 않으며, 기존 충돌과 warning은 서로 덮어쓰지 않는다.
