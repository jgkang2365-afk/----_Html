...
- 기능 요청은 화면·파일 단위로 즉시 축소하지 말고, 먼저 upstream → 현재 기능 → downstream과 Source of Truth·상태 전이를 확인한다.
- 세부 구현은 UI → API → 서버 → DB/RPC/worker의 실제 경로를 확인해 최소 범위로 수정한다.
- 새 필드·상태·역할·일정은 생성/수정/조회/요약/출력·후속 소비처까지 연동을 검토하고, 업무상 명백한 연동은 같은 작업에 포함한다. 정책 선택이 필요한 경우만 사용자에게 확인한다.
- 완료 전 전체 흐름에 다시 넣어 downstream 정합성, legacy/중복 원천, 고아 데이터, 회귀를 확인하며 화면 단위 PASS만으로 완료 처리하지 않는다.
