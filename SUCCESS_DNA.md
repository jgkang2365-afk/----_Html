...
### [2026-07-02] 사용자 정의 보고서 UI 컴포넌트 타입 에러 및 담당자 필드 누락
* **원인:**
...
* **해결법:**
  ...
  2. `preliminary_surveyor`, `actual_measurer`, `report_writer`, `measurer` 등의 데이터 필드를 `CustomQueryExport.tsx` 컴포넌트의 columns 설정 상태와 `FILTER_FIELDS` 배열에 추가하여 사용자가 상세 조건 쿼리 및 컬럼 숨김/순서 제어를 사용할 수 있도록 반영함.
