...
- [x] `lib/db/migrations/045_add_is_active_to_users.sql` 생성 및 실행.
    - `ALTER TABLE users ADD COLUMN is_active BOOLEAN DEFAULT true;`

### Phase 2: 백엔드 인증 및 API 강화
- [x] `app/api/auth/login/route.ts` 수정:
    - 사용자 조회 시 `is_active` 필드 포함.
    - `is_active`가 `false`인 경우 로그인 거부 및 안내 메시지 반환.
- [x] `app/api/users/route.ts` 및 `app/api/users/[id]/route.ts` 수정:
    - GET: `is_active` 필드 반환.
    - POST: 신규 생성 시 활성 상태 기본값 처리.
    - PATCH: `is_active` 상태 업데이트 지원.

### Phase 3: 관리자 UI 개선
- [x] `components/features/UserManagement.tsx` 수정:
...
## 4. UAT (사용자 수락 테스트)
- [x] 관리자가 사용자를 '비활성'으로 변경할 수 있는가?
- [x] '비활성' 상태인 사용자가 로그인을 시도했을 때 적절한 안내 메시지와 함께 차단되는가?
- [x] '비활성' 사용자를 다시 '활성'으로 변경했을 때 즉시 접속이 가능한가?
- [x] 계정 비활성 시에도 해당 사용자가 담당하던 사업장 정보가 그대로 유지되는가?
