-- Forward safety fix: assignment-only/date-only repair must preserve every existing surveyor snapshot exactly.
...
BEGIN
  ...
  FOR item IN SELECT value FROM jsonb_array_elements(p_repairs) ORDER BY (value->>'targetId')::bigint LOOP
    ...
    -- A repair that does not fill a field may never carry a divergent snapshot for that field.
    ...
    IF COALESCE((item->>'fillMeasurementAssignment')::boolean, false) THEN
      ...
      VALUES (repaired_plan.id, (expected_assignment->>'measurementDate')::date, (expected_assignment->>'assigneeUserId')::integer, upper(btrim(expected_assignment->>'surveyCode')), 'TRUE_CONFIRMED_MISSING_DOCUMENTARY_REPAIR_EXACT_RECONCILIATION');
    END IF;
