-- resource lock key JSON 연산자 우선순위를 명시한다. Forward-only, no backfill.
...
BEGIN
  ...
  corrected_definition := replace(current_definition,
    E'''reverse-planner-survey|phone|'' || plan_payload.value->>''preliminary_date'' || ''|'' || plan_payload.value->>''responsible_user_id''',
    E'''reverse-planner-survey|phone|'' || (plan_payload.value->>''preliminary_date'') || ''|'' || (plan_payload.value->>''responsible_user_id'')');
  corrected_definition := replace(corrected_definition,
    E'''reverse-planner-survey|field|'' || plan_payload.value->>''preliminary_date'' || ''|'' || participant.value',
    E'''reverse-planner-survey|field|'' || (plan_payload.value->>''preliminary_date'') || ''|'' || participant.value');
  corrected_definition := replace(corrected_definition,
    E'''reverse-planner-public-group|'' || assignment_payload.value->>''measurement_date'' || ''|'' || assignment_payload.value->>''assignee_user_id''',
    E'''reverse-planner-public-group|'' || (assignment_payload.value->>''measurement_date'') || ''|'' || (assignment_payload.value->>''assignee_user_id'')');

  IF corrected_definition = current_definition THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'REVERSE_PLANNER_LOCK_PRECEDENCE_PATCH_NOT_APPLIED';
  END IF;
