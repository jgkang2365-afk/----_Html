-- 추천 우선순위 점수는 조 단위가 될 수 있으므로 저장 컬럼과 RPC 인자를 BIGINT로 맞춘다.

DROP FUNCTION IF EXISTS public.persist_preliminary_survey_recommendation(
  BIGINT, TIMESTAMPTZ, TEXT, INTEGER, INTEGER, TEXT, DATE, INTEGER, DATE,
  TEXT, JSONB, INTEGER, JSONB, JSONB, INTEGER
);
DROP FUNCTION IF EXISTS public.persist_preliminary_survey_recommendation(
  BIGINT, TIMESTAMPTZ, TEXT, INTEGER, INTEGER, TEXT, DATE, INTEGER, DATE,
  TEXT, JSONB, BIGINT, JSONB, JSONB, INTEGER
);

ALTER TABLE public.preliminary_survey_plans
  ALTER COLUMN recommendation_score TYPE BIGINT;

CREATE FUNCTION public.persist_preliminary_survey_recommendation(
  p_target_id BIGINT,
