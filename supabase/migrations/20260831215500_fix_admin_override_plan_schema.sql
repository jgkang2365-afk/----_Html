-- 관리자 plan override를 실제 preliminary_survey_v2_plans 스키마에 맞춘다.
-- measurement_target_business.measurement_date(text)는 plan source_measurement_date(date)로 명시 변환한다.
...
BEGIN
  ...
  IF p_target_id IS NULL OR p_recommended_date IS NULL OR p_survey_method NOT IN ('field','phone')
     OR jsonb_typeof(p_participant_user_ids) <> 'array'
     OR jsonb_typeof(p_participant_names) <> 'array'
  ...
  SELECT count(DISTINCT value) INTO distinct_user_count
  FROM jsonb_array_elements_text(p_participant_user_ids);
  IF distinct_user_count <> jsonb_array_length(p_participant_user_ids) THEN
  ...
  END IF;
  IF target_row.measurement_date IS NULL OR target_row.measurement_date !~ '^\d{4}-\d{2}-\d{2}$' THEN
    RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'INVALID_SOURCE_MEASUREMENT_DATE';
  END IF;
  ...
  ELSE
    ...
    SET recommended_date = p_recommended_date,
        ...
        status = 'recommended',
        plan_origin = 'manual',
        source_measurement_date = target_row.measurement_date::date,
  ...
  INSERT INTO public.preliminary_survey_v2_admin_manual_override_audit (
    ...
    p_target_id, plan_row.id, 'plan', before_plan, to_jsonb(plan_row),
    COALESCE(p_policy_warnings,'[]'::jsonb), p_changed_by_user_id
  );
