-- worker가 선점하기 전(started_at IS NULL)의 오래된 K2B 취소 요청을 최종 취소한다.
...
BEGIN
  ...
  UPDATE public.background_jobs SET status = 'cancelled', finished_at = COALESCE(finished_at, CURRENT_TIMESTAMP), updated_at = CURRENT_TIMESTAMP
...
CREATE OR REPLACE FUNCTION public.claim_k2b_legacy_direct_job(p_payload JSONB)
RETURNS UUID
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE created_job_id UUID;
BEGIN
