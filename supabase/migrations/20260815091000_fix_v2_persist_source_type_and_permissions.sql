-- V2 저장 RPC source 타입 및 권한 수정 (20260815090000 후속)
-- 배경: measurement_target_business.measurement_date는 TEXT 컬럼이다.
--   → stale-source 검증은 "계산 당시 읽은 source 값과 현재 target 값이 같은가"를
--     원본 TEXT equality로 비교해야 하며, 날짜 cast(::date)를 하면
--     TEXT vs DATE 타입 불일치(42883)로 모든 실전 payload가 실패한다.
-- 1) batch RPC: source_measurement_date 비교에서 ::date cast 제거 (TEXT=text)
-- 2) 단건 RPC: p_source_measurement_date를 date → text 로 전환
--    (기존 date signature는 DROP하여 PostgREST overload ambiguity 방지)
-- 3) helper/batch/단건 RPC 권한을 PUBLIC/anon/authenticated에서 REVOKE하고
--    service_role에만 EXECUTE 부여

-- ---------- 1) 단건 RPC: date signature DROP 후 text signature 재생성 ----------
...
-- ---------- 2) batch RPC: source 비교에서 date cast 제거 ----------
...
BEGIN
  ...
  FOR plan IN SELECT value FROM jsonb_array_elements(p_plans) LOOP
    ...
    END IF;
    -- source stale 검증: 원본 TEXT equality (cast 금지). 다중 날짜 문자열도 그대로 비교 가능.
...
-- ---------- 3) 권한: PUBLIC/anon/authenticated 제거, service_role만 허용 ----------
