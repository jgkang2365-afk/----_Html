...
async function checkApiLogic() {
    ...
    if (allBusinessHistory) {
        allBusinessHistory.forEach(h => {
            console.log(`- ${h.year} ${h.period}: biz_num="${h.business_number}", name="${h.business_name}"`);
        });
    }
    ...
    console.log('\n--- Final Merged Result ---');
    console.log('Business Number:', business.business_number);
    console.log('Representative Name:', business.representative_name);
    console.log('Industrial Accident Number:', business.industrial_accident_number);
}
