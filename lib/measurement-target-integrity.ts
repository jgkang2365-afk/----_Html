...
export function inspectMeasurementTargetIntegrity(input: {
  ...
  const journalsById = new Map<string, IntegrityJournal>();
  for (const journal of input.journals) {
    journalsById.set(String(journal.id), journal);
  }
  ...
  for (const target of input.targets) {
    ...
    const targetHasCoordinates = hasIntegrityCoordinates(target);
    const infoHasCoordinates = hasIntegrityCoordinates(info);
    if (targetAddress && !targetHasCoordinates) {
      add(target, "REVIEW", "측정대상사업장 좌표 누락", `${display(target.latitude)}, ${display(target.longitude)}`, infoHasCoordinates
        ? `${info.latitude}, ${info.longitude}`
        : "measurement_target_business.latitude/longitude");
    }
    const targetGeocodingStatus = String(target.geocoding_status ?? "").toUpperCase();
    if (["FAILED", "ERROR", "ADDRESS_MISSING"].includes(targetGeocodingStatus) || String(target.geocoding_error ?? "").trim()) {
      add(target, "WARNING", "측정대상사업장 지오코딩 확인필요", target.geocoding_error ?? targetGeocodingStatus, targetGeocodingStatus || "오류 내용 확인");
    }
    if (targetHasCoordinates && !infoHasCoordinates) {
      add(target, "WARNING", "기준정보 좌표 확인필요", `${target.latitude}, ${target.longitude}`, "business_info.latitude/longitude");
    } else if (infoAddress && !infoHasCoordinates) {
    ...
    if (target.journal_id != null) {
      const linkedJournal = journalsById.get(String(target.journal_id));
      if (!linkedJournal) {
        add(target, "ERROR", "연결 일지 없음", target.journal_id, "measurement_journal.id");
      } else if (
        String(linkedJournal.code ?? "").trim() !== code
        || Number(linkedJournal.measurement_year) !== Number(target.year)
        || String(linkedJournal.measurement_period ?? "").trim() !== String(target.period ?? "").trim()
      ) {
        add(
          target,
          "ERROR",
          "일지 코드/연도/주기 불일치",
          `${code || "-"}/${target.year ?? "-"}/${target.period ?? "-"}`,
          `${display(linkedJournal.code)}/${linkedJournal.measurement_year ?? "-"}/${linkedJournal.measurement_period ?? "-"}`,
        );
      }
    }
