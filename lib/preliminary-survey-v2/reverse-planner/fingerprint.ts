...
export function sourceFingerprint(snapshot: PlanningSnapshot) {
  ...
  return deterministicHash({ canonicalSha: snapshot.canonicalSha, plannerVersion: snapshot.plannerVersion,
    ...
    existingSurveyOccupancy: snapshot.existingSurveyOccupancy,
    actualMeasurementOccupancy: snapshot.actualMeasurementOccupancy,
    existingPublicSampleAssignments: snapshot.existingPublicSampleAssignments });
}
