...
export async function validateManualPlanHardRules(
  ...
  if (!participantIds.has(input.target.responsible.id)) {
    errors.push("보고서 담당자는 예비조사자에 반드시 포함되어야 합니다.");
  }
  if (!input.target.responsible.experienced && !reviewer) {
    errors.push("비경력 보고서 담당자에게는 경력자 1명이 필요합니다.");
  }
  ...
  return { valid: errors.length === 0, errors, experiencedReviewer: reviewer, routeEvidence };
}
