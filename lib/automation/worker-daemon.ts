...
import { K2B_VERIFY_MANUAL_CANDIDATE_LIMIT } from '../constants/k2b-verification';
...
export class WorkerDaemon {
    ...
    private async processK2BVerifyJob(job: any) {
        ...
        const { data: datedJournals, error: datedJournalError } = await supabase.from('measurement_journal')
            ...
            .or('k2b_verified_status.is.null,k2b_verified_status.in.(UNVERIFIED,STALE,YELLOW)');
        // 직원이 K2B에서 직접 처리해 내부 전송일이 비어 있는 최근 후보도 별도 조회한다.
        // 이들은 정확히 한 건이 맞더라도 YELLOW만 기록하며 원래 입력값은 절대 바꾸지 않는다.
        const { data: manualCandidateJournals, error: manualCandidateError } = await supabase.from('measurement_journal')
            .select(journalFields)
            .is('k2b_send_date', null)
            .or('k2b_verified_status.is.null,k2b_verified_status.in.(UNVERIFIED,STALE,YELLOW)')
            .order('id', { ascending: false })
            .limit(K2B_VERIFY_MANUAL_CANDIDATE_LIMIT);
        if (datedJournalError || manualCandidateError) {
            await this.updateJobStatus(job.id, 'failed', datedJournalError?.message || manualCandidateError?.message || 'K2B 검증 대상을 조회하지 못했습니다.');
            return;
        }
        ...
        try {
            ...
            const reconciledWithJournal: { journal: any; item: ReturnType<typeof reconcileK2BSubmissionResults>[number] }[] = [];
            for (const [sendDate, dateJournals] of journalsBySendDate) {
                const remoteResults = await querySubmissionResultsForDate(sendDate);
                ...
                for (let index = 0; index < reconciled.length; index++) {
                    reconciledWithJournal.push({ journal: dateJournals[index], item: reconciled[index] });
                }
            }
            for (const { journal, item } of reconciledWithJournal) {
                ...
                if (item.match && !['AMBIGUOUS', 'NONE'].includes(item.matchMethod)) {
                    const { error: updateError } = await supabase.from('measurement_journal').update({
                        ...
                        k2b_verified_result_date: journal.k2b_send_date,
