...
export function toK2BExecutionStatus(job: K2BJobRow) {
  ...
  return {
    ...
    remoteExpectedRowCount: asNumber(result?.remoteExpectedRowCount),
    gridReadMethod: asString(result?.gridReadMethod),
    gridReadComplete: asString(result?.gridReadComplete),
